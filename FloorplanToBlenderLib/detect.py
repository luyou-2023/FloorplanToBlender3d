...
def outer_contours(detect_img, output_img=None, color=[255, 255, 255]):
    ...
    参数:
        @Param detect_img: 需要从中检测外轮廓的图像，必须提供。
        @Param output_img: 输出图像，默认为None，如果有提供，则会在该图像上绘制轮廓。
        @Param color: 设置在输出图像上的颜色，默认是白色 [255, 255, 255]。

    返回值:
        @Return approx: 近似轮廓的多边形顶点坐标。
        @Return box: 带有绘制轮廓的输出图像。

    来源:
        @Source https://stackoverflow.com/questions/50930033/drawing-lines-and-distance-to-them-on-image-opencv-python
    """

    # 对图像进行二值化处理，使用预定义的阈值范围
    # cv2.THRESH_BINARY_INV表示像素值大于阈值时将被设为0，否则设为最大值（即反转）
    ret, thresh = cv2.threshold(
        detect_img,
        const.OUTER_CONTOURS_TRESHOLD[0],  # 下阈值
        const.OUTER_CONTOURS_TRESHOLD[1],  # 上阈值
        cv2.THRESH_BINARY_INV,
    )

    # 查找轮廓，只保留最外层的轮廓（cv2.RETR_EXTERNAL），并且简化轮廓以节省内存（cv2.CHAIN_APPROX_SIMPLE）
    contours, hierarchy = cv2.findContours(
        thresh.copy(),  # 使用副本避免修改原始图像
        cv2.RETR_EXTERNAL,
        cv2.CHAIN_APPROX_SIMPLE
    )

    largest_contour_area = 0
    largest_contour = None  # 初始化最大轮廓变量
    for cnt in contours:
        # 找到面积最大的轮廓，认为它是代表平面图外侧边界的轮廓
        if cv2.contourArea(cnt) > largest_contour_area:
            largest_contour_area = cv2.contourArea(cnt)
            largest_contour = cnt

    # 根据预定义的精度计算epsilon，用于近似多边形的准确度
    epsilon = const.PRECISE_BOXES_ACCURACY * cv2.arcLength(largest_contour, True)
    # 使用epsilon对最大轮廓进行多边形近似
    approx = cv2.approxPolyDP(largest_contour, epsilon, True)

    # 如果提供了输出图像，则在上面绘制近似的多边形轮廓
    if output_img is not None:
        output_img = cv2.drawContours(output_img, [approx], -1, color, thickness=cv2.FILLED)  # 填充轮廓内部
